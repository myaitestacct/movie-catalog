...
export function isCompleteCastAnalytics(analytics) {
if (!analytics || typeof analytics !== 'object' || Array.isArray(analytics)) {
return false;
}
...
return isNumericValue(analytics.tagged_movies) &&
Number(analytics.tagged_movies) >= 0 &&
isNumericValue(analytics.untagged_movies) &&
Number(analytics.untagged_movies) >= 0 &&
isNumericValue(analytics.cast_assignments) &&
Number(analytics.cast_assignments) >= 0 &&
isNumericValue(analytics.unique_actors) &&
Number(analytics.unique_actors) >= 0 &&
isNumericValue(analytics.average_cast_size) &&
Number(analytics.average_cast_size) >= 0 &&
(analytics.top_actor === null || validActor(analytics.top_actor)) &&
Array.isArray(analytics.top_actors) &&
analytics.top_actors.length <= 20 &&
analytics.top_actors.every(validActor);
}

export function isCompleteMetadataCompleteness(analytics) {
if (!analytics || typeof analytics !== 'object' || Array.isArray(analytics)) {
return false;
}
...
field.label.trim() !== '' &&
isNumericValue(field.missing_count) &&
Number(field.missing_count) >= 0 &&
isNumericValue(field.complete_count) &&
Number(field.complete_count) >= 0;

return isNumericValue(analytics.total_movies) &&
Number(analytics.total_movies) >= 0 &&
isNumericValue(analytics.complete_movies) &&
Number(analytics.complete_movies) >= 0 &&
isNumericValue(analytics.incomplete_movies) &&
Number(analytics.incomplete_movies) >= 0 &&
Array.isArray(analytics.fields) &&
analytics.fields.length > 0 &&
analytics.fields.every(validField);
}
...
const castIsValid = data.cast_analytics === undefined ||
isCompleteCastAnalytics(data.cast_analytics);

const metadataCompletenessIsValid =
data.metadata_completeness === undefined ||
isCompleteMetadataCompleteness(data.metadata_completeness);
...
export async function fetchMetadataIssueRows(fieldKey, options = {}) {
...
if (!Array.isArray(data)) {
throw new ApiError('The metadata issue response is incomplete');
}
