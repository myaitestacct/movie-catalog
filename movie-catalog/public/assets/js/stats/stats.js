...
} from './stats-pagination.js';
import * as releaseYearAnalytics
    from './stats-release-years.js';
import * as genreAnalytics
    from './stats-genres.js';
import * as ratingRuntimeAnalytics
    from './stats-rating-runtime.js';
import * as certificationAnalytics
    from './stats-certifications.js';
import * as directorAnalytics
    from './stats-directors.js';
import * as castAnalytics
    from './stats-cast.js';
import * as languageCountryAnalytics
    from './stats-language-country.js';
import * as technicalFormatAnalytics
    from './stats-technical-formats.js';
import * as storageAnalytics
...
const statsRequests =
    createLatestRequest();
const statsDetailRequests =
    createLatestRequest();
const movieJumpRequests =
    createLatestRequest();
...
function bindLibraryIssueCard(
    issueType,
    config
) {
    const card =
        getLibraryIssueCard(config);

    if (
        !card ||
        card.dataset.libraryIssueBound ===
            'true'
    ) {
        return;
    }
...
export async function refreshStats() {
    ...
    try {
        ...
        const oldestYear =
            Number(data.oldest_year);

        const newestYear =
            Number(data.newest_year);
        ...
        languageCountryAnalytics
            .renderLanguageCountryAnalytics?.(
                data.language_country_analytics,
                data.total_movies
            );

        technicalFormatAnalytics
            .renderTechnicalFormatAnalytics?.(
        ...
        betterCopyCard.title =
            data.needs_better_copy_count > 0
                ? `Show ${
                    data.needs_better_copy_count
                } movies marked as needing ` +
                    'a better copy'
                : 'Check for movies marked ' +
                    'as needing a better copy';
...
function closePanel(
    ...
    statsToggleButton?.setAttribute(
        'aria-expanded',
        'false'
    );
}
...
async function jumpToMovie(num) {
    ...
    try {
        ...
        if (
            ...
            alert(
                'This movie is not included ' +
                'in the current filtered ' +
                'results. Clear or change ' +
