...
/**
 * Determine if a movie is an exact title match for the current search.
 * Mirrors backend: TRIM(FORMATTEDTITLE) = :exactTitle (case-insensitive)
 */
...
function createExactFuzzySeparator(columns, exactCount, fuzzyCount, fuzzy) {
  ...
  if (fuzzy) {
    text.textContent = `Exact matches (${exactCount}) • Fuzzy / Contains matches (${fuzzyCount})`;
  } else {
    text.textContent = `Exact matches (${exactCount}) • Other matches (${fuzzyCount})`;
  }
...
export function renderTable(table, rows, columns) {
  ...
  // Title exact vs fuzzy separator logic
  ...
  rows.forEach((movie, rowIndex) => {
    ...
    if (
      ...
      const prevIsExact =
        rowIndex > 0 ? isExactTitleMatch(rows[rowIndex - 1], exactTitle) : false;

      if (!isExact && (prevIsExact || rowIndex === exactCount)) {
        tbody.appendChild(
          createExactFuzzySeparator(columns, exactCount, fuzzyCount, fuzzy)
        );
