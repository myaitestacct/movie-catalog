...
'audio_formats' => $this->getConfiguredDelimitedFacet(
'audio_formats',
'AUDIOFORMAT',
$totalMovies
)
...
error_log('Cast analytics failed: ' . (string)$error);
return $this->buildCastAnalytics([], $totalMovies);
}
}
...
? round($facet['assignments'] / $taggedMovies, 1)
: 0.0,
'top_actor' => $facet['top_item'],

// A cast field can contain thousands of unique names. Keep the
// complete totals above while bounding the dashboard payload.
'top_actors' => array_slice($facet['items'], 0, 20)
];
}

private function getMetadataCompletenessAnalytics(int $totalMovies): array
{
$columns = array_map(
static fn(array $field): string => '`' . $field['column'] . '`',
self::METADATA_FIELDS
);
$query = $this->sql['metadata_values'] ??
'SELECT ' . implode(', ', $columns) . ' FROM movies;';
...
error_log('Metadata completeness failed: ' . (string)$error);
return $this->buildMetadataCompletenessAnalytics(
[],
$totalMovies
);
...
$unobservedMovies = max(0, $total - $observedMovies);
...
foreach (self::METADATA_FIELDS as $key => $field) {
if ($this->isMetadataValueMissing($row, $field)) {
$missingCounts[$key]++;
$missingField = true;
}
}
...
'complete_count' => max(0, $total - $missingCount)
];
}
...
'incomplete_movies' => max(0, $total - $completeMovies),
'fields' => $fields
];
}

private function isMetadataValueMissing(array $row, array $field): bool
{
...
private function metadataMissingSql(array $field): string
{
$column = '`' . $field['column'] . '`';
...
public function getMetadataIssueRows(string $fieldKey): array
{
$field = self::METADATA_FIELDS[$fieldKey] ?? null;

if ($field === null) {
throw new InvalidArgumentException('Unsupported metadata field');
}

$condition = $this->metadataMissingSql($field);
$query =
'SELECT `NUM`, `ORIGINALTITLE`, `FORMATTEDTITLE`, `YEAR`, `URL` ' .
'FROM movies WHERE ' . $condition . ' ' .
'ORDER BY COALESCE(NULLIF(TRIM(`ORIGINALTITLE`), \'\'), ' .
