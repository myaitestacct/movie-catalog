...
const applicationRoot =
  path.join(repositoryRoot, 'movie-catalog');
...
function createMovie(number, overrides = {}) {
  ...
  return {
    ...
    ORIGINALTITLE:
      overrides.ORIGINALTITLE || title,
    YEAR: String(1980 + (number % 45)),
    RATING:
      (6 + (number % 30) / 10).toFixed(1),
    FILESIZE:
      String(1_000_000_000 + number * 25_000_000),
    CERTIFICATION:
      number % 2 === 0 ? 'PG-13' : 'R',
    LENGTH:
      String(85 + (number % 55)),
    ...
    DESCRIPTION:
      `A deterministic description for ${title}.`,
    COUNTRY:
      number % 3 === 0
    ...
    DIRECTOR:
      number % 2 === 0
        ...
        : 'Devon Sample',
    ACTORS:
      'Alex Actor; Bailey Performer; Casey Star',
    SUBTITLES: 'English',
    PICTURENAME:
      `${title.toLowerCase().replaceAll(
        /[^a-z0-9]+/g,
...
movies[2] = createMovie(3, {
  ...
  DESCRIPTION:
    'A radio astronomer discovers alien signal.',
...
movies[4] = createMovie(5, {
  ...
  DESCRIPTION:
    'A nightclub owner must choose between love and virtue.',
...
function movieResponse(url) {
  ...
  const rawFilters =
    [...url.searchParams.entries()]
      .filter(
        ([key, value]) =>
          !reservedParameters.has(key) &&
          value.trim()
      );

  const mode =
    url.searchParams.get('mode') === 'OR'
      ? 'OR'
      : 'AND';

  const isFuzzy =
    url.searchParams.get('fuzzy') === 'true';

  const requestedTitleMode =
    url.searchParams.get('titleMode');
  ...
  const titleMode =
    [
      ...
      'CONTAINS',
      'FUZZY'
    ].includes(requestedTitleMode)
      ? requestedTitleMode
      : (isFuzzy ? 'FUZZY' : 'CONTAINS');

  const titleSearchRaw =
  ...
  const {
    title: exactTitle
  } = parseTitleSearch(titleSearchRaw);
    ...
    movies
      ...
      .toSorted((left, right) => {
        if (exactTitle) {
          ...
          const rightTitle =
            (right.FORMATTEDTITLE ?? '')
              .toString()
              .trim();

          const leftIsExact =
            leftTitle.toLowerCase() ===
            exactTitle.toLowerCase()
              ? 0
              : 1;
...
`;
  ...
  return (
    `${browserHeader}${stats}${movie}${browserFooter}`
  );
}
  ...
  switch (url.pathname) {
    ...
    default:
      sendJson(
        response,
        {
          error: 'Unknown mock API endpoint'
        },
        404
      );
  }
...
server.listen(port, host, () => {
  console.log(
    `Movie Catalog browser-test server listening on http://${host}:${port}`
  );
});
